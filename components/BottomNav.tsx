...
const NavItem: React.FC<{
  ...
  >
    ...
    <div className={`relative z-10 transition-all duration-300 ${isActive ? '-translate-y-1.5' : 'group-hover:-translate-y-1'}`}>
      <Icon className={`w-5 h-5 transition-colors duration-300 ${isActive ? 'text-yellow-400' : 'text-gray-400 group-hover:text-gray-200'}`} />
    </div>

    {/* Label */}
    <span className={`relative z-10 text-[9px] font-bold uppercase tracking-widest transition-all duration-300 ${isActive ? 'text-yellow-400 opacity-100 translate-y-0.5' : 'text-gray-500 opacity-0 translate-y-2 group-hover:opacity-60 group-hover:translate-y-1'}`}>
      {label}
    </span>
  </button>
);
...
const BottomNav: React.FC<BottomNavProps> = ({ activeView, setActiveView }) => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[92%] max-w-lg z-40">
      <nav className="h-14 bg-gray-900/80 backdrop-blur-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)] rounded-[1.5rem] px-3 overflow-hidden">
